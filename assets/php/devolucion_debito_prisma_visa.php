...
class devolucion_debito_prisma_visa extends devolucion_debito {
    ...
    private function crear_detalle($registro) {
        ...
        $result["PeriodoMes"] = substr($registro, 52, 2);
        $result["PeriodoAno"] = "20" . substr($registro, 64, 2);
        $result["FechaOrigen"] = substr($registro, 50, 6);          // - FechaOrigen        51      6      N       DDMMAA                      Fecha de Origen
        $result["Monto"] = substr($registro, 62, 15);               // - Importe            63     15      N       n13d2                       Importe (13 dígitos enteros y 2 decimales con ceros a izquierda)
        $result["Cuotas"] = substr($registro, 77, 2);               // - Cuotas             78      2      N       
        $result["Identificador"] = substr($registro, 94, 15);       // - Identificador      95     15      AN
        $result["Socio"] = substr($registro, 94, 15);               // - Identificador      95     15      AN
        ...
        $result["CodRechazo"] = substr($registro, 130, 2);          // - RechazoCodMot1    131      2      AN                                  Rechazo Código Motivo 1
        $result["DescripcionRechazo"] = substr($registro, 132, 29); // - RechazoDescr1     133     29      AN                                  Descripción motivo de rechazo 1
